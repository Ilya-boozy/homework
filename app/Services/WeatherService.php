...
class WeatherService
{
    public static function get_weather()
    {
        ...
        $weather_api_url = "https://api.weather.yandex.ru/v2/informers?lat=" . $weather_lat . "&lon=" . $weather_lon;
        $request         = Http::withHeaders(["X-Yandex-API-Key" => $weather_key])->get($weather_api_url);
        if ($request->successful()) {
            $request = $request->json();

            return "temperature in Bryansk is " . $request["fact"]["temp"];
        }

        return "something goes wrong " . $request["fact"]["temp"];
    }
}